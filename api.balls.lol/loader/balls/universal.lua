...
-- === Universal feature set (paste AFTER: local main = Library.new()) ===
...
-- Tabs
local generalTab = main:create_tab("General", "rbxassetid://12345678")
local visualsTab = main:create_tab("Visuals", "rbxassetid://87654321")
local miscTab    = main:create_tab("Misc",    "rbxassetid://11223344")

-- ---------- Movement Module ----------
local moveModule = generalTab:create_module({
    title = "Movement",
    description = "Movement utilities (Jump, Fly, NoClip, Speed)",
    section = "left",
    flag = "movement_module",
    callback = function(state) end
})

-- Infinite Jump
moveModule:create_checkbox({
    title = "Infinite Jump",
    flag = "inf_jump",
    callback = function(state)
        if state then
            if getgenv()._infJumpConn and getgenv()._infJumpConn.Connected then
                getgenv()._infJumpConn:Disconnect()
            end

            getgenv()._infJumpConn = UserInputService.JumpRequest:Connect(function()
...
moveModule:create_checkbox({
    ...
    callback = function(state)
        if state then
            if getgenv()._noclipConn and getgenv()._noclipConn.Connected then
                getgenv()._noclipConn:Disconnect()
            end

            getgenv()._noclipConn = RunService.Stepped:Connect(function()
                pcall(function()
                    local plr = Players.LocalPlayer
            ...
            -- best-effort restore collisions on player's character
            pcall(function()
                local plr = Players.LocalPlayer
                if plr and plr.Character then
                    for _, part in ipairs(plr.Character:GetDescendants()) do
...
-- Fly (WASD + Space / LeftCtrl). Robust fly using BodyVelocity + BodyGyro
moveModule:create_checkbox({
    ...
    callback = function(state)
        ...
        local function stopFly()
            if getgenv()._flyConn then
                pcall(function() getgenv()._flyConn:Disconnect() end)
                getgenv()._flyConn = nil
            end
            if getgenv()._flyBV then
                pcall(function() getgenv()._flyBV:Destroy() end)
                getgenv()._flyBV = nil
            end
            if getgenv()._flyBG then
                pcall(function() getgenv()._flyBG:Destroy() end)
                getgenv()._flyBG = nil
            end
        end
        ...
        if state then
            ...
            getgenv()._flySpeed = getgenv()._flySpeed or 50 -- default speed (customizable)

            -- Heartbeat movement loop
            getgenv()._flyConn = RunService.Heartbeat:Connect(function()
                ...
                if moveVec.Magnitude > 0 then
                    bv.Velocity = moveVec.Unit * getgenv()._flySpeed
                    -- rotate body gyro to face camera direction (keeps orientation stable)
...
-- Speed override and speed value textbox
moveModule:create_checkbox({
    ...
    callback = function(state)
        local plr = Players.LocalPlayer
        local hum = plr.Character and plr.Character:FindFirstChildOfClass("Humanoid")
        local speedVal = tonumber(Library._config._flags["speed_value"]) or 50

        if state then
            if getgenv()._speedOverrideConn and getgenv()._speedOverrideConn.Connected then
                getgenv()._speedOverrideConn:Disconnect()
            end
            if hum then
                pcall(function() hum.WalkSpeed = speedVal end)
            end
            getgenv()._speedOverrideConn = RunService.Heartbeat:Connect(function()
                if hum and hum.WalkSpeed ~= speedVal then
                    pcall(function() hum.WalkSpeed = speedVal end)
                end
            end)
        else
            if getgenv()._speedOverrideConn then
                getgenv()._speedOverrideConn:Disconnect()
                getgenv()._speedOverrideConn = nil
            end
            if hum then
                pcall(function() hum.WalkSpeed = 16 end) -- reset to default
            end
        end
    end
})

moveModule:create_textbox({
    title = "Speed Value (number)",
    placeholder = "50",
    flag = "speed_value",
    callback = function(text)
        local num = tonumber(text)
        if num then
            if Library._config._flags["speed_override"] then
                local plr = Players.LocalPlayer
                local hum = plr.Character and plr.Character:FindFirstChildOfClass("Humanoid")
                if hum then pcall(function() hum.WalkSpeed = num end) end
            end
        end
    end
})
...
-- ---------- Visuals Module ----------
local visualsModule = visualsTab:create_module({
    title = "Visuals",
    description = "ESP and camera tweaks",
    section = "left",
    flag = "visuals_module",
    callback = function(state) end
})

visualsModule:create_checkbox({
    title = "ESP (Name tags)",
    flag = "esp",
    callback = function(state)
        local function cleanup()
            for _, pl in ipairs(Players:GetPlayers()) do
                if pl.Character and pl.Character:FindFirstChild("Head") then
                    local g = pl.Character.Head:FindFirstChild("UniversalESP")
        ...
        if state then
            if getgenv()._espConn and getgenv()._espConn.Connected then
                getgenv()._espConn:Disconnect()
            end

            getgenv()._espConn = RunService.RenderStepped:Connect(function()
                for _, v in ipairs(Players:GetPlayers()) do
                    if v ~= Players.LocalPlayer and v.Character and v.Character:FindFirstChild("Head") then
                        if not v.Character.Head:FindFirstChild("UniversalESP") then
...
visualsModule:create_textbox({
    title = "Camera FOV (number)",
    placeholder = "70",
    flag = "fov_value",
    callback = function(text)
        local num = tonumber(text)
        if num and workspace.CurrentCamera then
            pcall(function() workspace.CurrentCamera.FieldOfView = num end)
        end
    end
})

-- ---------- Misc Module ----------
local miscModule = miscTab:create_module({
    title = "Misc",
    description = "Utilities: GodMode, Anti-AFK, Teleport, BunnyHop",
    section = "left",
    flag = "misc_module",
    callback = function(state) end
})

-- GodMode: keep humanoid health at max each frame (best-effort)
miscModule:create_checkbox({
    title = "GodMode (heal loop)",
    flag = "godmode",
    callback = function(state)
        local plr = Players.LocalPlayer
        if state then
            if getgenv()._godConn and getgenv()._godConn.Connected then
                getgenv()._godConn:Disconnect()
            end
            getgenv()._godConn = RunService.Heartbeat:Connect(function()
                local hum = plr.Character and plr.Character:FindFirstChildOfClass("Humanoid")
                if hum then
                    pcall(function()
                        if hum.MaxHealth and hum.Health < hum.MaxHealth then
            ...
            if getgenv()._godConn then
                getgenv()._godConn:Disconnect()
                getgenv()._godConn = nil
            end
        end
    end
})
...
miscModule:create_checkbox({
    ...
    callback = function(state)
        ...
        if state then
            if getgenv()._afkConn and getgenv()._afkConn.Connected then
                getgenv()._afkConn:Disconnect()
            end
            getgenv()._afkConn = localPlayer.Idled:Connect(function()
                pcall(function()
...
-- Teleport to player (enter player name in textbox then press Enter)
miscModule:create_textbox({
    title = "Teleport to player (enter name + Enter)",
    ...
    callback = function(text)
        ...
        if not target or not target.Character then
            warn("Teleport: target not found or has no character")
            return
        end
...
miscModule:create_checkbox({
    ...
    callback = function(state)
        local plr = Players.LocalPlayer
        if state then
            if getgenv()._bHopConn and getgenv()._bHopConn.Connected then
                getgenv()._bHopConn:Disconnect()
            end
            getgenv()._bHopConn = RunService.Heartbeat:Connect(function()
                local hum = plr.Character and plr.Character:FindFirstChildOfClass("Humanoid")
                if hum then
            ...
            if getgenv()._bHopConn then
                getgenv()._bHopConn:Disconnect()
                getgenv()._bHopConn = nil
            end
        end
    end
})
